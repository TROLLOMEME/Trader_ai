...
with open("keypair.json") as f:
    secret = json.load(f)
keypair = Keypair.from_bytes(bytes(bytearray(secret)))
...
def get_jupiter_tokens():
    res = requests.get(JUPITER_TOKENS_URL)
    tokens = res.json()
    return [t for t in tokens if t.get("symbol") and t.get("mint") and t.get("chainId") == 101][:50]

def analyze_tokens_with_gpt(tokens):
    symbols = ", ".join([t["symbol"] for t in tokens])
    prompt = f"From these Solana tokens: {symbols}, which one looks best to buy today? Only return the SYMBOL."
    res = openai.ChatCompletion.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "You are a Solana trading bot."},
            {"role": "user", "content": prompt}
        ]
    )
    return res.choices[0].message.content.strip().upper()

def get_jupiter_quote(output_mint, sol_amount):
    params = {
        "inputMint": SOL_MINT,
...
def simulate_trade(token):
    ...
    if "data" not in quote or not quote["data"]:
        print(f"[ERROR] No route to {token['symbol']}")
        return
    best = quote["data"][0]
    out_amount = int(best["outAmount"]) / (10 ** token["decimals"])
    print(f"[SIMULATED TRADE] Buy {out_amount:.4f} {token['symbol']} for {MAX_SOL_PER_TRADE} SOL")

while True:
    print("[INFO] Getting token list from Jupiter...")
    tokens = get_jupiter_tokens()

    print("[INFO] Asking GPT for best token...")
    symbol = analyze_tokens_with_gpt(tokens)

    selected = next((t for t in tokens if t["symbol"].upper() == symbol), None)
    if selected:
        simulate_trade(selected)
    else:
        print(f"[WARN] Token '{symbol}' not found.")
